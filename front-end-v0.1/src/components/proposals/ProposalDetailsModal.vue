  ...
  <div v-if="isOpen" class="modal-overlay" @click.self="emitClose">
    <div class="modal-container" @click.stop>
      <div class="modal-header">
        <h3>Proposta #{{ proposal?.idProposta }}</h3>
        <button class="close-btn" @click="emitClose" title="Fechar">
          <i class="bi bi-x"></i>
        </button>
      </div>

      <div class="modal-body" v-if="proposal">
        <div class="info-grid">
          <div class="info-field">
            <label>ID Proposta</label>
            <p class="info-value mono">{{ proposal.idProposta }}</p>
          </div>
          <div class="info-field">
            <label>Empresa</label>
            <p class="info-value">{{ proposal.empresa?.nomeFantasia || proposal.empresa?.razaoSocial || '—' }}</p>
          </div>
          <div class="info-field">
            <label>Valor</label>
            <p class="info-value">{{ formatCurrency(proposal.valorProposta) }}</p>
          </div>
          <div class="info-field">
            <label>Prazo de Validade</label>
            <p class="info-value">{{ formatDate(proposal.prazoValidade) }}</p>
          </div>
          <div class="info-field">
            <label>Status</label>
            <span :class="['status-badge', getStatusClass(proposal.statusProposta)]">{{ displayStatus(proposal.statusProposta) }}</span>
          </div>
          <div class="info-field">
            <label>Data de Criação</label>
            <p class="info-value">{{ formatDate(proposal.dataCriacao) }}</p>
          </div>
        </div>

        <div v-if="proposal.driveLink" class="document-section">
          <h4>Documento</h4>
          ...
          <div class="iframe-container">
            <iframe 
              :src="getEmbedUrl(proposal.driveLink)" 
              frameborder="0"
  ...
  methods: {
    ...
    getStatusClass(status) {
      switch (status) {
        ...
        case 'Em_analise': return 'status-review'
        case 'Recusada': return 'status-rejected'
        default: return ''
      }
    },
    getEmbedUrl(url) {
    ...
    downloadDocument() {
      if (!this.proposal?.driveLink) return
      
      let downloadUrl = this.proposal.driveLink
